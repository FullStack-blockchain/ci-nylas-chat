...
function callFoldersTable($sPaginationType, $iDisplayLength, $fnSortcol, $fnSortdir, $dirpaginate){
    ...
    $('#sortable_filter input').on( 'keyup', function () {
          tableSearch(fTable, this.value);
    });
}
...
function callFilesTable($sPaginationType, $bPaginate, $bFilter, $iDisplayLength, $fnSortcol, $fnSortdir){
    ...
    // accent insensitive search
    $('#sort_filter input').on('keyup', function () {
        
          tableSearch(oTable, this.value);
    });
}

// close multiple downoad modal after click
$(document).on('click', '.downzip', function() {
    $('#zipmodal').modal('hide');
});

$(document).on('click', '.bigzip a', function(e) {
    e.preventDefault();
    var loc = $(this).attr('href');
    var newicon = '<i class="fa fa-files-o fa-2x fa-fw"></i> <i class="fa fa-angle-double-right fa-2x fa-fw passing-animated"></i> <i class="fa fa-file-archive-o fa-2x fa-fw"></i>';
    $(this).html(newicon).attr('href', '#').addClass('disabled');
    location.href = loc;
});
