...
$(function () {
    ...
    if (calendar_selector.length > 0) {
        ...
        var calendar_settings = {
            themeSystem: $("#calendar_page").length > 0 ? false : 'bootstrap3',
            customButtons: {},
            header: $("#calendar_page").length > 0 ? false : {
            ...
            viewRender: function (view) {
                ...
                if ($("#calendar_page").length > 0) {
                    ...
                    $('#calendar_page > .page-header').removeClass(currentMonthShort1);
                    currentMonthShort1 = calendar1.getDate().format('MMM');
                    $('#calendar_page > .page-header').addClass(currentMonthShort1);
                }
        ...
        $('#calendar_page .page-header .change-view').click(function () {
            calendar1.changeView($(this).data('view'));
        });
    ...
    if ($('#sales-upload').length > 0) {
        new Dropzone('#sales-upload', $.extend({}, _dropzone_defaults(), {
            ...
            success: function (files, response) {
                ...
                var data = '';
                if (response.success === true || response.success == 'true') {
                    data += '<div class="display-block full-width sales-attach-file-preview" data-attachment-id="' + response.attachment_id + '">';
...
function alert_float(type, message) {
    if (type == 'success') {
        PNotify.success({title: 'Success!', text: message})
    }

    if (type == 'danger') {
        PNotify.error({title: 'Oh No!', text: message})
    }

    if (type == 'warning') {
        PNotify.notice({title: 'Warning!', text: message})
    }
}
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
    ...
    get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
        ...
        table_row += '<td><div class="btn-group"><a href="#" class="btn btn-danger pull-left min-height-auto m-t-2" onclick="delete_item(this,' + itemid + '); return false;"><i class="fa fa-trash s-4"></i></a></div></td>';
