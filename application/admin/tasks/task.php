            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        ?>
                        <?php if (isset($task)) { ?>
                            <div class="pull-right mb-2 task-single-menu task-menu-options">
                                    ...
                                    <ul class="action-ul">
                                        <?php if (has_permission('tasks', '', 'create')) { ?>
                                            ...
                                            if (total_rows('tblstafftaskassignees', array('taskid' => $task->id)) > 0) {
                                                $copy_template .= '<div class="form-check"><label class="form-check-label"><input class="form-check-input" type="checkbox" name="copy_task_assignees" id="copy_task_assignees" checked/><span class="checkbox-icon"></span><span class="form-check-description" for="copy_task_assignees">' . _l('task_single_assignees') . '</span></label></div>';
                                            }
                                            if (total_rows('tblstafftasksfollowers', array('taskid' => $task->id)) > 0) {
                                                $copy_template .= '<div class="form-check"><label class="form-check-label"><input class="form-check-input" type="checkbox" name="copy_task_followers" id="copy_task_followers" checked/><span class="checkbox-icon"></span><span class="form-check-description" for="task_single_followers">' . _l('task_single_followers') . '</span></label></div>';
                                            }
                                            if (total_rows('tbltaskchecklists', array('taskid' => $task->id)) > 0) {
                                                $copy_template .= '<div class="form-check"><label class="form-check-label"><input class="form-check-input" type="checkbox" name="copy_task_checklist_items" id="copy_task_checklist_items" checked/><span class="checkbox-icon"></span><span class="form-check-description" for="copy_task_checklist_items">' . _l('task_checklist_items') . '</span></label></div>';
                                            }
                                            if (total_rows('tblfiles', array('rel_id' => $task->id, 'rel_type' => 'task')) > 0) {
                                                $copy_template .= '<div class="form-check"><label class="form-check-label"><input class="form-check-input" type="checkbox" name="copy_task_attachments" id="copy_task_attachments" checked/><span class="checkbox-icon"></span><span class="form-check-description" for="copy_task_attachments">' . _l('task_view_attachments') . '</span></label></div>';
                                            }

                                            $copy_template .= "<p class='mt-2'>" . _l('task_status') . "</p>";
                                            $task_copy_statuses = do_action('task_copy_statuses', $task_statuses);
                                            foreach ($task_copy_statuses as $copy_status) {
                                                $copy_template .= '<div class="form-check"><label class="form-check-label"><input type="radio" class="form-check-input" value="' . $copy_status["id"] . '" name="copy_task_status" id="copy_task_status_' . $copy_status["id"] . '" ' . ($copy_status["id"] == do_action("copy_task_default_status", 1) ?  "checked" : "") .'/><span class="radio-icon"></span><span for="copy_task_status_' . $copy_status["id"] . '">' . $copy_status["name"] . '</span></label></div>';
                                            }
                                ...
                                <?php if (has_permission('tasks', '', 'delete') || has_permission('tasks', '', 'create')) { ?>
                                    <a href="#" onclick="return false;"
                                       class="trigger manual-popover btn btn-icon m-r-5">
                        ...
                        <div class="pull-left mb-2">
                            <div class="form-check form-check-inline task-add-edit-public">
                                <label class="form-check-label">
                                    <input class="form-check-input" type="checkbox" id="task_is_public"
                                           name="is_public" <?php if (isset($task)) {
                                        if ($task->is_public == 1) {
                                            echo 'checked';
                                        }
                                    }; ?>/>
                                    <span class="checkbox-icon"></span>
                                    <span class="form-check-description" for="task_is_public" data-toggle="tooltip"
                                          data-placement="bottom"
                                          title="<?php echo _l('task_public_help'); ?>"><?php echo _l('task_public'); ?></span>
                                </label>
                            </div>
                            <div class="form-check form-check-inline task-add-edit-billable">
                                <label class="form-check-label">
                                    <input class="form-check-input" type="checkbox" id="task_is_billable"
                                           name="billable"
                                    ...
                                    <span class="form-check-description"
                                          for="task_is_billable"><?php echo _l('task_billable'); ?></span>
                                </label>
                            </div>
                            <div class="form-check form-check-inline task-visible-to-customer <?php if ((isset($task) && $task->rel_type != 'project') || !isset($task) || (isset($task) && $task->rel_type == 'project' && total_rows('tblprojectsettings', array('project_id' => $task->rel_id, 'name' => 'view_tasks', 'value' => 0)) > 0)) {
                                echo ' hide';
                            } ?>">
                                <label class="form-check-label">
                                    <input class="form-check-input" type="checkbox" id="task_visible_to_client"
                                    ...
                                    <span class="checkbox-icon"></span>
                                    <span class="form-check-description"
                                          for="task_visible_to_client"><?php echo _l('task_visible_to_client'); ?></span>
                                </label>
                            </div>
                        </div>
                        <?php if (!isset($task)) { ?>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <span class="input-group-btn">
                                                         <button class="btn btn-success add_more_attachments p-1"
                                                                 type="button"><i
                        ...
                        <div class="task-hours<?php if (isset($task) && $task->rel_type == 'project' && total_rows('tblprojects', array('id' => $task->rel_id, 'billing_type' => 3)) == 0) {
                            echo ' hide';
                        } ?>">
                        ...
                        <?php echo render_textarea('description', '', (isset($task) ? $task->description : ''), array('rows' => 6, 'placeholder' => _l('task_add_description'), 'data-task-ae-editor' => true, !is_mobile() ? 'onclick' : 'onfocus' => (!isset($task) || isset($task) && $task->description == '' ? 'init_editor(\'.tinymce-task\', {height:200, auto_focus: true});' : '')), array(), 'no-margin no-padding-top', 'tinymce-task'); ?>
                    </div>
                </div>
            </div>
