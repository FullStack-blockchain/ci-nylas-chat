...
* Update LANG
*/
if (isset($_GET['languagemanager']) && GateKeeper::canSuperAdmin('superadmin_can_translations')) {
...
} elseif (isset($_GET['users']) && GateKeeper::canSuperAdmin('superadmin_can_users')) {
...
} elseif (isset($_GET['log']) && GateKeeper::canSuperAdmin('superadmin_can_statistics')) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            */
            if (isset($_FILES['file']['name']) && $_FILES['file']['error'] !== 4) {
                ...
                $extension = strtolower(pathinfo($logonew, PATHINFO_EXTENSION));
            ...
            */
            ...
            $removelogo = filter_input(
                INPUT_POST, "removelogo", FILTER_SANITIZE_STRING
            );

            if (!$logonew && $removelogo) {
                $_CONFIG['logo'] = false;
            } else {
            ...
            $ip_list = filter_input(
                INPUT_POST, "ip_list", FILTER_SANITIZE_STRING
            );
            $ip_redirect = filter_input(
                INPUT_POST, "ip_redirect", FILTER_VALIDATE_URL
            );
            $ip_blacklist = filter_input(
                INPUT_POST, "ip_blacklist", FILTER_SANITIZE_STRING
            );
            $ip_whitelist = filter_input(
                INPUT_POST, "ip_whitelist", FILTER_SANITIZE_STRING
            );
            ...
            $_CONFIG['recaptcha_site'] = filter_input(
                INPUT_POST, "recaptcha_site", FILTER_SANITIZE_STRING
            );
            $_CONFIG['recaptcha_secret'] = filter_input(
                INPUT_POST, "recaptcha_secret", FILTER_SANITIZE_STRING
            );
