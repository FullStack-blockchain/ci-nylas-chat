...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<div class="form-check"><label class="form-check-label"><input class="form-check-input" type="checkbox" value="' . $aRow['id'] . '"><span class="checkbox-icon"></span><span class="form-check-description"></span></label></div>';
...
*/
    ...
    if ($canChangeStatus) {
        ...
        $outputStatus .= '<ul class="dropdown-menu dropdown-menu-right width250" aria-labelledby="tableTaskStatus-' . $aRow['id'] . '">';
    ...
    if (has_permission('tasks', '', 'edit') && $aRow['status'] != 5) {
        $outputPriority .= '<div class="inline-block mleft5 table-export-exclude">';
        $outputPriority .= '<div class="dropdown">';
        $outputPriority .= '<a style="font-size:14px;vertical-align:middle;" class="dropdown-toggle text-dark" id="tableTaskPriority-' . $aRow['id'] . '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
        $outputPriority .= '<span data-toggle="tooltip" title="' . _l('task_single_priority') . '">&nbsp;&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></span>';
        $outputPriority .= '</a>';

        $outputPriority .= '<div class="dropdown-menu dropdown-menu-right" aria-labelledby="tableTaskPriority-' . $aRow['id'] . '">';
        foreach($tasksPriorities as $priority){
            if ($aRow['priority'] != $priority['id']) {
                $outputPriority .= '
                  <a href="#" class="dropdown-item" onclick="task_change_priority(' . $priority['id'] . ',' . $aRow['id'] . '); return false;">
                     ' . $priority['name'] . '
                  </a>';
            }
        }
