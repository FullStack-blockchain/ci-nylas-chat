...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'subject') {
            ...
            $_data = '<div class="form-check ">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" data-switch-url="' . admin_url() . 'surveys/change_survey_status" name="onoffswitch" class="onoffswitch-checkbox" id="c_' . $aRow['surveyid'] . '" data-id="' . $aRow['surveyid'] . '" ' . $checked . '>
