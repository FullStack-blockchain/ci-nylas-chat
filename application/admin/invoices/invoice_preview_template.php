...
<?php if (count($invoices_to_merge) > 0) { ?>
    <div class="panel_s no-padding card mb-4">
...
<div class="col-md-12 no-padding">
    <div class="panel-body card">
        ...
        <div class="row">
            <div class="col-md-3">
                <?php echo format_invoice_status($invoice->status, 'mt-2'); ?>
            ...
            <div class="col-md-9 _buttons">
                ...
                <div class="pull-right">
                    ...
                    <?php if (has_permission('invoices', '', 'edit')) { ?>
                        <div class="btn-group ml-1">
                            <a href="<?php echo admin_url('invoices/invoice/' . $invoice->id); ?>" data-toggle="tooltip"
                               ...
                               data-placement="bottom"><i class="fa fa-pencil-square-o s-4"></i></a>
                        </div>
                    <?php } ?>
                    <div class="btn-group ml-1">
                        ...
                        <ul class="dropdown-menu dropdown-menu-right width200">
                            ...
                            <li>
                                <a href="<?php echo admin_url('invoices/pdf/' . $invoice->id); ?>"><?php echo _l('download'); ?></a>
                            </li>
                            <li>
                    ...
                    <?php if (!empty($invoice->clientid)) { ?>
                        <div class="btn-group ml-1" <?php if ($invoice->status == '5') { ?> data-toggle="tooltip" data-title="<?php echo _l('invoice_cancelled_email_disabled'); ?>"<?php } ?>>
                            <a href="#"
                               class="invoice-send-to-client btn-with-tooltip btn btn-default<?php if ($invoice->status == 5) {
                                   echo ' disabled';
                    ...
                    <?php if (has_permission('payments', '', 'create') && abs($invoice->total) > 0) { ?>
                        <div class="btn-group ml-1">
                            <a href="#" onclick="record_payment(<?php echo $invoice->id; ?>); return false;"
                               class="btn btn-success text-white <?php if ($invoice->status == 2 || $invoice->status == 5) {
                                   echo 'disabled';
                               } ?>">
                                <i class="fa fa-plus-square text-white s-4"></i>&nbsp;&nbsp;<?php echo _l('payment'); ?></a>
                        </div>
                    <?php } ?>
                </div>
            </div>
        ...
        <div class="tab-content">
                                    ...
                                    <td><?php echo format_money($credit['amount'], $invoice->symbol) ?>
                                        <?php if (has_permission('credit_notes', '', 'delete')) { ?>
                                            <a href="<?php echo admin_url('credit_notes/delete_invoice_applied_credit/' . $credit['id'] . '/' . $credit['credit_id'] . '/' . $invoice->id); ?>"
                                               class="pull-right text-danger _delete"><i class="fa fa-trash s-4"></i></a>
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php } ?>
            ...
            <div role="tabpanel" class="tab-pane" id="tab_reminders">
                <a href="#" class="btn btn-secondary btn-xs" data-toggle="modal"
                   data-target=".reminder-modal-invoice-<?php echo $invoice->id; ?>"><i
                            class="fa fa-bell-o s-4"></i> <?php echo _l('invoice_set_reminder_title'); ?></a>
                <hr class="mt-4"/>
                <?php render_datatable(array(_l('reminder_description'), _l('reminder_date'), _l('reminder_staff'), _l('reminder_is_notified')), 'reminders'); ?>
                <?php $this->load->view('admin/includes_fuse/modals/reminder', array('id' => $invoice->id, 'name' => 'invoice', 'members' => $members, 'reminder_title' => _l('invoice_set_reminder_title'))); ?>
            </div>
            <?php if ($invoice->recurring > 0) { ?>
            ...
            <div role="tabpanel" class="tab-pane" id="tab_notes">
                ...
                <div class="text-right">
                    <button type="submit"
                            class="btn btn-info mt-4 mb-4"><?php echo _l('estimate_add_note'); ?></button>
                </div>
                <?php echo form_close(); ?>
                <hr class="mb-4"/>
                <div class="panel_s mt-4 no-shadow" id="sales_notes_area"></div>
            </div>
