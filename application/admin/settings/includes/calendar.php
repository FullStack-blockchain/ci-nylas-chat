<ul class="nav nav-tabs" role="tablist">
    <li role="presentation" class="nav-item active">
        <a href="#general" aria-controls="general" role="tab" class="nav-link"
           data-toggle="tab"><?php echo _l('settings_group_general'); ?></a>
    </li>
    <li role="presentation" class="nav-item">
        <a href="#colors" aria-controls="colors" role="tab" class="nav-link"
    ...
    <div role="tabpanel" class="tab-pane active" id="general">
        ...
        <div class="form-group">
            ...
            <select class="selectpicker" data-width="100%" name="settings[default_view_calendar]"
                ...
                <option value="month"<?php if (get_option('default_view_calendar') == 'month') {
                    echo ' selected';
                } ?>><?php echo _l('month'); ?></option>
                <option value="basicWeek"
                "<?php if (get_option('default_view_calendar') == 'basicWeek') {
                    echo ' selected';
                } ?>><?php echo _l('week'); ?></option>
                <option value="basicDay"
                "<?php if (get_option('default_view_calendar') == 'basicDay') {
                    echo ' selected';
                } ?>><?php echo _l('day'); ?></option>
                <option value="agendaWeek"
                "<?php if (get_option('default_view_calendar') == 'agendaWeek') {
                    echo ' selected';
                } ?>><?php echo _l('agenda'); ?> <?php echo _l('week'); ?></option>
                <option value="agendaDay"
                "<?php if (get_option('default_view_calendar') == 'agendaDay') {
                    echo ' selected';
