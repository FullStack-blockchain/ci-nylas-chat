...
<ul class="nav nav-tabs" role="tablist">
    <li role="presentation" class="nav-item active">
        <a href="#set_invoice" aria-controls="set_invoice" role="tab" class="nav-link"
    ...
    <li class="nav-item" role="presentation">
        <a href="#expenses" class="nav-link" aria-controls="expenses" role="tab"
           data-toggle="tab"><?php echo _l('expenses'); ?></a>
    </li>
    <li class="nav-item" role="presentation">
        <a href="#contracts" class="nav-link" aria-controls="contracts" role="tab"
           data-toggle="tab"><?php echo _l('contracts'); ?></a>
    </li>
    <li class="nav-item" role="presentation">
        <a href="#tasks" class="nav-link" aria-controls="tasks" role="tab"
           data-toggle="tab"><?php echo _l('tasks'); ?></a>
    </li>
    <li class="nav-item" role="presentation">
        <a href="#tickets" class="nav-link" aria-controls="tickets" role="tab"
           data-toggle="tab"><?php echo _l('tickets'); ?></a>
    </li>
    <li class="nav-item" role="presentation">
        <a href="#surveys" class="nav-link" aria-controls="surveys" role="tab"
    ...
    <div role="tabpanel" class="tab-pane active" id="set_invoice">
        <?php if (!is_invoices_overdue_reminders_enabled()) { ?>
            <div class="alert alert-warning">
                The system was not able to find sources to send overdue notices, if you want overdue notices to be sent,
                make sure that in <a href="<?php echo admin_url('emails'); ?>">email templates</a> the overdue notice
                email for invoices is enabled or at least you have configured <a
                        href="<?php echo admin_url('settings?group=sms'); ?>">SMS</a> overdue notice.
            </div>
        <?php } ?>
        ...
        <hr class="mt-4 mb-4"/>
        <h4 class="mt-4 font-medium"><?php echo _l('invoices_list_recurring'); ?></h4>


        <div class="form-check">
            <label class="form-check-label">
                <input type="radio" id="generate_and_send" name="settings[new_recurring_invoice_action]" class="form-check-input"
                       value="generate_and_send"<?php if (get_option('new_recurring_invoice_action') == 'generate_and_send') {
                    echo ' checked';
                } ?>>
                <span class="radio-icon"></span>
                <span><?php echo _l('reccuring_invoice_option_gen_and_send'); ?></span>
            </label>
        </div>

        <div class="form-check">
            <label class="form-check-label">
                <input type="radio" id="generate_unpaid_invoice" name="settings[new_recurring_invoice_action]" class="form-check-input"
                       value="generate_unpaid"<?php if (get_option('new_recurring_invoice_action') == 'generate_unpaid') {
                    echo ' checked';
                } ?>>
                <span class="radio-icon"></span>
                <span><?php echo _l('reccuring_invoice_option_gen_unpaid'); ?></span>
            </label>
        </div>

        <div class="form-check">
            <label class="form-check-label">
                <input type="radio" id="generate_draft_invoice" name="settings[new_recurring_invoice_action]" class="form-check-input"
                       value="generate_draft"<?php if (get_option('new_recurring_invoice_action') == 'generate_draft') {
                    echo ' checked';
                } ?>>
                <span class="radio-icon"></span>
                <span><?php echo _l('reccuring_invoice_option_gen_draft'); ?></span>
            </label>
        </div>
