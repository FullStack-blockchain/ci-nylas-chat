...
<?php echo form_close(); ?>
<label class="mt-2 mb-1"><?php echo _l('project_file_visible_to_customer'); ?></label>
<div class="onoffswitch">
    <label class="switch onoffswitch-label" for="pf_visible_to_customer">
            ...
            <div class="modal-body">
                    ...
                    <div class="form-check">
                        <label class="form-check-label">
                            ...
                            <span class="form-check-description"
                                  for="mass_delete"><?php echo _l('mass_delete'); ?></span>
                        </label>
                    </div>
                    <hr class="mt-4 mb-4"/>
                <?php } ?>
                <div id="bulk_change">
                    <div class="form-group pt-0">
                        <label class="mb-1"><?php echo _l('project_file_visible_to_customer'); ?></label>
                        <div class="onoffswitch">
                            <label class="switch onoffswitch-label" for="bulk_pf_visible_to_customer">
    ...
    <tr>
        <th data-orderable="false"><span class="hide"> - </span>
            <div class="form-check mass_select_all_wrap">
                <label class="form-check-label">
                    <input class="form-check-input" type="checkbox" id="mass_select_all" data-to-table="project-files">
                    <span class="checkbox-icon"></span>
                    <span class="form-check-description"></span>
                </label>
            </div>
        </th>
    ...
    <?php foreach ($files as $file) {
        ...
        <tr>
            <td>
                <div class="form-check">
                    <label class="form-check-label">
                        <input class="form-check-input" type="checkbox" value="<?php echo $file['id']; ?>">
                        <span class="checkbox-icon"></span>
                        <span class="form-check-description"></span>
                    </label>
                </div>
            </td>
            <td data-order="<?php echo $file['file_name']; ?>" class="ellipsis">
                <a href="#"
                   onclick="view_project_file(<?php echo $file['id']; ?>,<?php echo $file['project_id']; ?>); return false;">
                    <?php if (is_image(PROJECT_ATTACHMENTS_FOLDER . $project->id . '/' . $file['file_name']) || (!empty($file['external']) && !empty($file['thumbnail_link']))) {
                        echo '<div class="text-left"><i class="fa fa-spinner fa-spin mt-6"></i></div>';
                        echo '<img class="project-file-image img-table-loading mr-2" src="#" data-orig="' . project_file_url($file, true) . '" width="100">';
                        echo '</div>';
                    }
                    echo $file['subject']; ?></a>
            </td>
            ...
            <td data-order="<?php echo $file['visible_to_customer']; ?>">
                ...
                ?>
                <div class="onoffswitch">
                    <label class="switch onoffswitch-label" for="<?php echo $file['id']; ?>">
            ...
            <td nowrap>
                <?php if (empty($file['external'])) { ?>
                    <button type="button" data-toggle="modal"
                            ...
                            data-target="#send_file" class="btn btn-info btn-icon btn-icon-square"><i class="fa fa-envelope s-4"></i>
                    </button>
                <?php } ?>
                <?php if ($file['staffid'] == get_staff_user_id() || has_permission('projects', '', 'delete')) { ?>
                    <a href="<?php echo admin_url('projects/remove_file/' . $project->id . '/' . $file['id']); ?>"
                       class="btn btn-danger btn-icon btn-icon-square _delete ml-1"><i class="fa fa-remove s-4"></i></a>
                <?php } ?>
            </td>
        </tr>
    <?php } ?>
