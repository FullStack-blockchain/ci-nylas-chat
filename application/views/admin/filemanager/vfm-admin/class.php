    ...
    */
    public static function showThumbnail($file, $inline = false)
    {
        ...
        if (!file_exists($thumbpath)) {
            ...
            $imageout = $image ? $image : imagecreatefromjpeg('vfm-admin/images/placeholder.jpg');
    ...
    */
    public function updateUser(
        ...
        if (GateKeeper::isUser($postoldname, $postoldpass)) {
            ...
            $jcustomfields = $_POST['user-customfields'];
            if ($jcustomfields) {
        ...
        foreach ($users as $user) {
            if (isset($user['sensitive']) && $user['sensitive'] === $userName) {
                if (crypt($passo, $user['pass']) == $user['pass']) {
                    ...
                    Utils::setWarning($encodeExplorer->getString('your_new_username_is').' <strong>'.$user['name'].'</strong>');
                    return true;
                }
                break;
            }
        ...
        if (Actions::fileExists($upload_file)) {
            ...
            if (!$location->editAllowed() || !$location->isWritable()) {
                ...
                Utils::setError(
                    '<span><i class="fa fa-exclamation-triangle"></i> '
                    .'<strong>.'.$name.'</strong> :'
                    .$encodeExplorer->getString('file_extension_missing').'</span> '
                );
...
class Utils
{
    ...
    */
    public static function checkIP()
    {
        ...
        if ($ip_list) {
            if ($ip_list === 'allow' && Utils::notList($guest_ip, SetUp::getConfig('ip_whitelist'))) {
                $kickoff = true;
            }

            if ($ip_list === 'reject' && Utils::inList($guest_ip, SetUp::getConfig('ip_blacklist'))) {
                $kickoff = true;
            }
            if ($kickoff) {
    ...
    public function createFileFromChunks($location, $temp_dir, $fileName, $chunkSize, $totalSize, $logloc)
    {
        ...
        if ($total_files * $chunkSize >= ($totalSize - $chunkSize + 1)) {
            ...
            if (is_dir($upload_dir)
                ...
                $chunk->setError(' <span><i class="fa fa-exclamation-triangle"></i> cannot create the destination file inside <strong>'.$upload_dir.'</strong>', 'nope');
                return false;
            }
        }
    }
