...
class Newsfeed extends Admin_controller
{
    ...
    public function load_newsfeed()
    {
        ...
        foreach ($posts as $post) {
            ...
            $response .= '<div class="newsfeed_post' . $pinned_class . '" data-main-postid="' . $post['postid'] . '">';
            ...
            $response .= '<a href="' . admin_url('profile/' . $post['creator']) . '">' . staff_profile_image($post['creator'], [
                'staff-profile-image-small avatar',
                'no-radius',
            ]) . '</a>';

            $response .= '</div>';
            $response .= '<div class="media-body">';
            $response .= '<p class="media-heading no-margin"><a href="' . admin_url('profile/' . $post['creator']) . '">' . get_staff_full_name($post['creator']) . '</a></p>';
            ...
            if ($post['creator'] == get_staff_user_id() || is_admin()) {
                ...
                $response .= '<button class="btn btn-default dropdown-toggle btn-post-options" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"><i class="fa fa-ellipsis-v"></i></button>';
            ...
            $response .= '<small class="post-time-ago pull-right">' . time_ago($post['datecreated']) . '</small>';
            $response .= '<div class="clear-fix"></div></div>';
    ...
    public function init_post_likes($id)
    {
        ...
        if (!$this->newsfeed_model->user_liked_post($id)) {
            $_likes .= '<button type="button" class="btn btn-default btn-icon-square" onclick="like_post(' . $id . ')"> <i class="fa fa-heart"></i></button>';
        } else {
            $_likes .= '<button type="button" class="btn btn-danger btn-icon-square" onclick="unlike_post(' . $id . ')"> <i class="fa fa-heart-o"></i></button>';
        }
        $_likes .= '</div>';
    ...
    public function comment_single($comment)
    {
        ...
        if ($total_comment_likes > 0) {
            $_comment_likes = ' - ' . $likes_modal . $total_comment_likes . ' <i class="fa fa-thumbs-o-up width-auto"></i></a>';
        } else {
            $_comment_likes .= '</a>';
        }
    ...
    public function get_data()
    {
        ...
        $this->load->view('admin/includes_fuse/modals/newsfeed_form', $data);
    }
