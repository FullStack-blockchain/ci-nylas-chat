<?php if ((credits_can_be_applied_to_invoice($invoice->status) && $credits_available > 0)) { ?>
    <!-- Modal Apply Credits -->
    <div class="modal fade apply-credits-from-invoice" id="apply_credits"
         data-balance-due="<?php echo $invoice->total_left_to_pay; ?>" tabindex="-1" role="dialog"
         aria-labelledby="modalLabelApplyCredits">
        <div class="modal-dialog modal-lg" role="document">
            <?php echo form_open(admin_url('invoices/apply_credits/' . $invoice->id), array('id' => 'apply_credits_form')); ?>
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title"
                        id="modalLabelApplyCredits"><?php echo format_invoice_number($invoice->id); ?>
                        - <?php echo _l('apply_credits'); ?></h4>
                    ...
                    <div class="table-responsive credits-table">
                        <table class="table table-bordered no-mtop">
                            ...
                            <?php foreach ($open_credits as $credit) { ?>
                                <tr>
                                    <td>
                                        <a href="<?php echo admin_url('credit_notes/list_credit_notes/' . $credit['id']); ?>"
                                           target="_blank"><?php echo format_credit_note_number($credit['id']); ?></a>
                                    </td>
                                    ...
                                    <td>
                                        <input type="number" max="<?php echo $credit['available_credits']; ?>"
                                               name="amount[<?php echo $credit['id']; ?>]"
                                               class="form-control apply-credits-field" value="0">
                                    </td>
                                </tr>
                            <?php } ?>
                            </tbody>
